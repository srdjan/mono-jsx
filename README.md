...
mono-jsx uses the same JSX syntax as React, but with a few differences.
...
#### Using Pseudo Class and Media Query in `style` Property

mono-jsx allows you to use pseudo classes and media queries in the `style` property.
...
mono-jsx uses [`<slot>`](https://developer.mozilla.org/en-US/docs/Web/HTML/Element/slot) element to render the slot content.(Equivalent to React's `children` prop)
...
> Note: the `html` tag function is **unsafe** that can cause [**XSS**](https://en.wikipedia.org/wiki/Cross-site_scripting) vulnerabilities.
...
function App() {
  return (
    <div>
      ...
      <strong>
        <use-state name="counter" value={0} />
      </strong>
      <button onClick={() => state.counter++}>+</button>
    </div>
  );
...
function App() {
  return (
    <div>
      <use-state switch name="image" value={0}>
        <Image1 />
        <Image2 />
        <Image3 />
      </use-state>
...
You can add an unique `key` to each child element as the `switch` component uses the key to identify the child element to render.

```jsx
function App() {
  return (
    <div>
      <use-state switch name="image" value="image-1">
        <Image1 key="image-1" />
        <Image2 key="image-2" />
        <Image3 key="image-3" />
      </use-state>
